...
3. [Day 1: Local Development Deployment](#day-1-local-development-deployment)
4. [Day 1: Production Deployment (Terraform Lifecycle)](#day-1-production-deployment-terraform-lifecycle)
5. [Day 2: Operations Runbooks](#day-2-operations-runbooks)
...
```
                         ┌─────────────────────────────────────────────┐
                         │         EventBridge (S3 Landing Trigger)      │
                         └──────────────────┬──────────────────────────┘
                                              │
       ┌──────────────┐    ┌───────────────┐ ▼ ┌───────────────────┐
       │  Bronze (Raw) │───▶│  GX Validator  │──▶│  Silver (Clean)   │
  ...
  │  ┌──────────┐   ┌──────────┐   ┌──────────┐   ┌────────────────┐  │
  │  │Ingestion │──▶│ Profiling │──▶│ Quality  │──▶│  Cataloging    │  │
  │  │  Node    │   │   Node    │   │  Router  │   │  Node (LLM)    │  │
  │  └──────────┘   └──────────┘   └────┬─────┘   └───────┬────────┘  │
  │                                     │                  │           │
  │                              ┌──────▼──────┐           │           │
  │                              │ Log & Retry │           │           │
